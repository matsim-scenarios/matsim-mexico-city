 ...
 * @author mrieser
 */
public final class MexicoCityAbsoluteFareRoadPricingTollCalculator implements LinkEnterEventHandler, VehicleEntersTrafficEventHandler, VehicleLeavesTrafficEventHandler {
	 ...
	 */
	private final NavigableMap<Id<Person>, AgentTollInfo> agents = new TreeMap<>();
	...
	Logger log = LogManager.getLogger( MexicoCityAbsoluteFareRoadPricingTollCalculator.class ) ;

	MexicoCityAbsoluteFareRoadPricingTollCalculator(final Network network, final RoadPricingScheme scheme, EventsManager events, final Population population) {
...
//	}
	...
	/**
	 * returns The toll paid by all the agents.
	 */
	...
	/**
	 * returns The Number of all the Drawees.
	 */
	int getDraweesNr() {
		int dwCnt = 0;
		for (AgentTollInfo ai : this.agents.values()) {
	...
	private class DistanceTollBehaviour implements TollBehaviourI {
		...
		/**
		 * events :The EventsManager to send money events.
		 */
		DistanceTollBehaviour(EventsManager events) {
			this.events = events;
		}
		@Override
		public void handleEvent(final LinkEnterEvent event, final Link link) {
			...
			if (cost != null) {
				...
				AgentTollInfo info = MexicoCityAbsoluteFareRoadPricingTollCalculator.this.agents.computeIfAbsent( driverId, (key) -> new AgentTollInfo() );
				info.toll += newToll;
			}
		}
	}
	...
	private final class LinkTollBehaviour implements TollBehaviourI {
		private final EventsManager events;
		/**
		 * events: The EventsManager to send money events.
		 */
	 ...
	 * </ul>
	 */
	private final class AreaTollBehaviour implements TollBehaviourI {
		private final EventsManager events;
		/**
		 * the EventsManager to send money events.
		 */
...
//			TODO: use vehicle type instead of vehicle id to detect mode
			if (event.getVehicleId().toString().contains(TransportMode.car)) {
				...
				Cost cost = MexicoCityAbsoluteFareRoadPricingTollCalculator.this.scheme.getLinkCostInfo(link.getId(), event.getTime(), driverId, event.getVehicleId());
				if (cost != null) {
					AgentTollInfo info = MexicoCityAbsoluteFareRoadPricingTollCalculator.this.agents.get(driverId);
					if (info == null) {
						...
						/* Register it. */
						MexicoCityAbsoluteFareRoadPricingTollCalculator.this.agents.put(driverId, info);
