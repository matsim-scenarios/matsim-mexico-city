...
public class RunMexicoCityScenario extends MATSimApplication {
	...
	protected Config prepareConfig(Config config) {
		...
		config.scoring().addActivityParams(new ScoringConfigGroup.ActivityParams("freight_start").setTypicalDuration((double) 60 * 15));
		config.scoring().addActivityParams(new ScoringConfigGroup.ActivityParams("freight_end").setTypicalDuration((double) 60 * 15));
