...
import static org.matsim.prepare.population.MexicoCityUtils.roundCoord;
...
public class ExtractFacilityShp implements MATSimAppCommand {
	...
	@CommandLine.Mixin
	private CrsOptions crs = new CrsOptions("EPSG:4326", RunMexicoCityScenario.CRS);
	...
	private void processIntersection(List<Feature> list, STRtree index) {
		...
		while (it.hasNext()) {
			...
			for (Feature other : query) {
				...
				used = checkIfIntersect(other, ft, used);
			}
	...
	private static boolean checkIfIntersect(Feature other, Feature ft, boolean used) {
		try {
	...
	private void process(OsmEntity entity) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (c != null && (c.values.containsKey("*") || c.values.containsKey(tag.getValue()))) {
				filtered = false;
				break;
			}
		}
		...
		checkIfOSMNode(entity, n);
	}

	private void checkIfOSMNode(OsmEntity entity, int n) {
