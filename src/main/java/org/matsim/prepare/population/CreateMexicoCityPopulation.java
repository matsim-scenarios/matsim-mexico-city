...
public class CreateMexicoCityPopulation implements MATSimAppCommand {
	...
	@CommandLine.Option(names = "--year", description = "Year to use statistics from", defaultValue = "2017")
	private int year;
	...
	private final CoordinateTransformation ct = new GeotoolsTransformation("EPSG:4326", "EPSG:4326");
	...
	public Integer call() throws Exception {
		...
		if (shp.getShapeFile() == null) {
			//TODO: define whether to use manzanas or some bigger zones.. zones should be size-wise comparable to Berlin LOR
		...
		try (CSVParser reader = new CSVParser(Files.newBufferedReader(input, Charset.forName("windows-1252")), format.build())) {
			...
			for (CSVRecord row : reader) {
				...
				//TODO: replace "Jahr" with pendent in mx data
				...
				try {
					...
					log.error("Error processing manzana", e);
	...
	private void processLOR(CSVRecord row) throws ParseException {
		...
		//TODO: adapt names of rows here to mx data row names
		...
		try {
			...
			unemployed = 0;
			log.warn("Manzana {} {} has no unemployment", raumID, row.get(1));
		}
		...
		for (int i = 0; i < n * sample; i++) {
			...
			Coord coord = ct.transform(sampleHomeCoordinate(geom, "EPSG:4326", landuse, rnd));
	...
	public static Coord sampleHomeCoordinate(MultiPolygon geometry, String crs, LanduseOptions landuse, SplittableRandom rnd) {
		...
		return RunMexicoCityCalibration.roundCoord(coord);
	}
