 ...
 */
public final class MexicoCitySumoNetworkHandler extends DefaultHandler {
	...
	private MexicoCitySumoNetworkHandler() {}
     ...
     */
	static MexicoCitySumoNetworkHandler read(File file) throws ParserConfigurationException, SAXException, IOException {
		SAXParserFactory factory = SAXParserFactory.newInstance();
		SAXParser saxParser = factory.newSAXParser();
		MexicoCitySumoNetworkHandler sumoHandler = new MexicoCitySumoNetworkHandler();
     ...
     */
	void merge(MexicoCitySumoNetworkHandler other, CoordinateTransformation ct) {
		...
		switch (qName) {
			...
			case "location":
				...
				case "lane":
					...
					if (attributes.getValue("allow") != null) {
						allow.addAll(Arrays.asList(StringUtils.explode(attributes.getValue("allow"), ' ')));
					}

					if (attributes.getValue("disallow") != null) {
						disallow.addAll(Arrays.asList(StringUtils.explode(attributes.getValue("disallow"), ' ')));
					}
	...
	static final class Edge {
		...
		Edge(String id, String from, String to, String type, String name, String[] shape) {
			this.id = id;
			this.from = from;
			this.to = to;
			this.type = type;
	...
	static final class Lane {
		...
		Lane(String id, int index, double length, double speed, Set<String> allow, Set<String> disallow) {
			this.id = id;
			this.index = index;
			this.length = length;
			this.speed = speed;
